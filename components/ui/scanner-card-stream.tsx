...
const ScannerCardStream = ({
  ...
  useEffect(() => {
    ...
    const handleMouseDown = (e: MouseEvent | TouchEvent) => {
      ...
      if (cardLine) cardLine.style.cursor = 'grabbing';
    };
    ...
    const handleMouseMove = (e: MouseEvent | TouchEvent) => {
      ...
      cardStreamState.current.velocity = Math.abs(delta) / 0.016;
    ...
    const handleMouseUp = () => {
      ...
      if (cardLine) cardLine.style.cursor = 'grab';
    };

    const handleWheel = (e: WheelEvent) => {
      e.preventDefault();
      cardStreamState.current.velocity = Math.min(Math.abs(e.deltaX || e.deltaY) * 2, 800);
      cardStreamState.current.direction = (e.deltaX || e.deltaY) > 0 ? -1 : 1;
    };
  ...
  return (
    <div className="relative w-full h-[250px] overflow-hidden">
      ...
      {showSpeed && (
        <div className="absolute top-3 right-4 z-30 text-xs font-mono text-white/40">
          {speed} px/s
        </div>
      )}

      {showControls && (
        <div className="absolute top-3 left-4 z-30 flex gap-2">
          <button onClick={toggleAnimation} className="text-xs font-mono text-white/60 hover:text-white px-2 py-1 border border-white/10 rounded">
            {isPaused ? 'Play' : 'Pause'}
          </button>
          <button onClick={changeDirection} className="text-xs font-mono text-white/60 hover:text-white px-2 py-1 border border-white/10 rounded">
            Flip
          </button>
          <button onClick={resetPosition} className="text-xs font-mono text-white/60 hover:text-white px-2 py-1 border border-white/10 rounded">
            Reset
          </button>
        </div>
      )}
            ...
            <div key={card.id} className="card-wrapper relative w-[400px] h-[250px] shrink-0">
              ...
              >
                <img
                  src={card.image}
                  alt="Card"
                  className="w-full h-full object-cover rounded-[15px] transition-all duration-300 ease-in-out brightness-110 contrast-110 hover:brightness-125 hover:contrast-125"
                />
              </div>
              <div
              ...
              >
                <pre
                  className="ascii-content absolute top-0 left-0 w-full h-full text-[rgba(220,210,255,0.6)] font-mono text-[11px] leading-[13px] overflow-hidden whitespace-pre m-0 p-0 text-left align-top box-border animate-glitch"
                  style={{
